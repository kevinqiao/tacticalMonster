...
export const checkCasualGameTimeoutAndIngest = internalAction({
  ...
  handler: async (ctx, { gameRowId, gameId, uid }) => {
    ...
    if (!settled.ok || !settled.shouldIngest) {
      console.log("[match3] timeout job skip", gameId, settled);
    ...
    if (!ingest.ok) {
      console.warn("[match3] timeout ingest failed", gameId, ingest.error);
      return { ok: false as const, error: ingest.error };
    }
    console.log("[match3] timeout ingest ok", gameId, ingest.parsed.deduped);
