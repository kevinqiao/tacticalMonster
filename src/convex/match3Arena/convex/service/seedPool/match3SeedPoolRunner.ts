...
export type GeneratePoolOptions = {
  ...
  rolloutCount: number;
  matchSeconds: number;
  /** Reject opening boards with fewer legal swaps (开局保活). */
  minOpeningMoves: number;
  minScoreP25: number;
  /**
   * Reject seeds where too many rollouts die in the early-move window.
   * Mid/late stuck is allowed and does not use this gate.
   */
  maxEarlyStuckRate: number;
  /** Reject seeds with almost no early cascade feel. */
  minEarlyCascadeHitRate: number;
  /** Reject likely_dead layouts after rollout metrics. */
  rejectLikelyDead: boolean;
};
...
export function processOneSeed(
  ...
  try {
    ...
    if (metrics.earlyStuckRate > options.maxEarlyStuckRate) {
      return {
        kind: "rejected",
        entry: {
          seedId,
          reason: "high_early_stuck_rate",
          detail: metrics.earlyStuckRate.toFixed(3),
        },
      };
    }
    if (metrics.earlyCascadeHitRate < options.minEarlyCascadeHitRate) {
      return {
        kind: "rejected",
        entry: {
          seedId,
          reason: "low_early_cascade_hit_rate",
          detail: metrics.earlyCascadeHitRate.toFixed(3),
        },
      };
    }
    if (options.rejectLikelyDead && metrics.layoutOutcome === "likely_dead") {
      return {
        kind: "rejected",
        entry: { seedId, reason: "likely_dead_layout", detail: metrics.layoutOutcome },
      };
    }
...
/** Defaults: 开局保活 gate on; mid-game stuck still allowed in sims. */
