...
export const importSeedPoolBegin = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert("match3_seed_pool_meta", {
...
async function writeSeedPoolBatch(
  ...
  if (!meta && createMetaIfMissing) {
    const metaId = await ctx.db.insert("match3_seed_pool_meta", {
      poolVersion,
      rolloutCount: 40,
  ...
  for (const e of entries) {
    ...
    const existing = await ctx.db
      .query("match3_seed_pool_entries")
    ...
    if (existing) {
      ...
      await ctx.db.insert("match3_seed_pool_entries", {
  ...
  if (indexOnly !== true && rollouts.length > 0) {
    for (const r of rollouts) {
      ...
      const existing = await ctx.db
        .query("match3_seed_pool_rollout_summaries")
      ...
      if (existing) {
        ...
        await ctx.db.insert("match3_seed_pool_rollout_summaries", doc);
      }
    }
  }
...
export const upsertRolloutsForSeed = internalMutation({
  ...
  handler: async (ctx, { poolVersion, seedId, rollouts }) => {
    ...
    for (const r of rollouts) {
      ...
      await ctx.db.insert("match3_seed_pool_rollout_summaries", {
        poolVersion: r.poolVersion,
        seedId: r.seedId,
        rolloutIndex: r.rolloutIndex,
        finalScore: r.finalScore,
