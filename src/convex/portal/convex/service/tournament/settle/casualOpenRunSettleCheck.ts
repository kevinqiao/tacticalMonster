...
export async function scheduleOpenRunSettleCheckForPlayerGame(
  ctx: MutationCtx,
  args: {
    playerGameId: Id<"portal_run_player_games">;
    gameId: string;
    uid: string;
    gameType: string;
    createdAt: number;
  ...
  try {
    ...
    console.warn("[portal] scheduleOpenRunSettleCheck failed", args.gameId, e);
  }
}
...
export async function cancelOpenRunSettleCheckForGameId(
  ...
  try {
    ...
    console.warn("[portal] cancelOpenRunSettleCheck failed", gameId, e);
  }
