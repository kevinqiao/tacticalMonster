...
import { resolveAsyncLeaderboardRowState } from "../../../../data/portalAsyncLeaderboardRowState";
import {
...
export type CasualAsyncTableSummary = {
  maxPlayers: number;
  rows: CasualAsyncTableLeaderboardRow[];
  /** true:?? confirmed/settled + bot ? reveal ???;?????,????? poll */
  isBoardStable?: boolean;
  /** ????:?????(? watchContext);`rows` ???? */
  triathlonLegs?: CasualTriathlonLegTableSummary[];
};

type PlayerMatchRow = Doc<"portal_run_player_matches">;
...
export async function buildCasualAsyncTableSummary(
  ...
  args: {
    ...
    maxPlayers: number;
    matchId: string;
    /** false:??? settled;??????????/??,? open/replaying ?? Playing */
    allHumansSettled?: boolean;
    /** ??????:?????,?? DB ?????,?? bot reveal ?? */
    historical?: boolean;
  }
