...
export type WebMerchantStaffCandidate = {
  uid: string;
  partnerId: number;
};

/** All Web identities for accountId — merchant console may sign in without ?partnerId (pid=0). */
export const listWebMerchantStaffCandidates = internalQuery({
  args: {
    accountId: v.string(),
  ...
  handler: async (ctx, { accountId, partnerId }) => {
    ...
    if (!subject) return [] as WebMerchantStaffCandidate[];

    const seen = new Set<string>();
    const out: WebMerchantStaffCandidate[] = [];
