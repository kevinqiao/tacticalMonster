...
export const create = internalMutation({
    ...
    handler: async (ctx, { uid,token,name,avatar }) => {
        const pid = await ctx.db.insert("tm_player", { uid,token,name,avatar,level:0,exp:0 });
        return pid;
    },
})
...
export const find = internalQuery({
    ...
    handler: async (ctx, { uid }) => {
        const player = await ctx.db.query("tm_player").withIndex("by_uid", (q) => q.eq("uid", uid)).unique();
        return player;
    },
})
