...
async function expireCasualMatchQueueRow(
...
): Promise<{ matchId: string; templateId: string } | null> {
  ...
  const matchId = await tryOpenSoloFallbackFromExpiredQueueRow(ctx, queueRowId);
  if (!matchId) return null;
  return { matchId, templateId: row.templateId };
}
...
async function tryOpenMultiTableBatch(
  ...
  const def = getTournamentDefinition(templateId);
  if (!def || !Number.isFinite(requiredHumans) || requiredHumans < 2) return null;
  ...
  if (batchSize < requiredHumans) return null;
  ...
  const matchId = await openTableForClaimedRows(ctx, {
    templateId,
    claimedRows,
    charged,
    instanceId,
...
export async function processCasualMatchQueueForTemplateCore(
  ...
  for (let round = 0; round < maxRounds; round++) {
    let openedMatchId: string | null = null;
    ...
    for (const [need, rows] of byEffective) {
      if (rows.length >= need) {
        const matchId = await tryOpenMultiTableBatch(ctx, templateId, need, rows);
        if (matchId) {
    ...
    if (!openedMatchId) return openedMatchIds;
    openedMatchIds.push(openedMatchId);
  }
  return openedMatchIds;
}

async function bindCasualMatchSeedAfterOpen(
  ctx: ActionCtx,
  matchId: string,
  templateId: string
  ...
  if (def.gameType !== "solitaire" && def.gameType !== "block_blast") {
    return { ok: true as const };
  }
  ...
  if (!bind.ok) {
    console.error("[casual] bindCasualMatchSeed failed after async open", {
      matchId,
      templateId,
      error: bind.error,
    });
...
export const processCasualMatchQueueForTemplate = internalMutation({
  ...
  handler: async (ctx, { templateId }) => {
    const matchIds = await processCasualMatchQueueForTemplateCore(ctx, templateId);
    return { ok: true as const, matchIds };
  },
});

/** 开桌后同步 bind seed（与 solo `joinTournament` 一致） */
export const processCasualMatchQueueForTemplateAndBind = internalAction({
  args: { templateId: v.string() },
  handler: async (ctx, { templateId }) => {
    const { matchIds } = await ctx.runMutation(
      internal.service.tournament.join.casualMatchmaking.processCasualMatchQueueForTemplate,
      { templateId }
    );
    for (const matchId of matchIds) {
      await bindCasualMatchSeedAfterOpen(ctx, matchId, templateId);
    }
    return { ok: true as const, matchIds };
  },
});
...
export const openSoloTableFromScheduledQueue = internalMutation({
  ...
  handler: async (ctx, { queueRowId }) => {
    const row = await ctx.db.get(queueRowId);
    if (!row) return { ok: true as const, matchId: undefined };
    const matchId = await tryOpenSoloTableFromQueueRow(ctx, row.templateId, row);
    return { ok: true as const, matchId, templateId: row.templateId };
  },
});

/** 单真人异步桌：开桌后同步 bind seed */
export const openSoloTableFromScheduledQueueAndBind = internalAction({
  args: { queueRowId: v.id("casual_match_queue") },
  handler: async (ctx, { queueRowId }) => {
    const result = await ctx.runMutation(
      internal.service.tournament.join.casualMatchmaking.openSoloTableFromScheduledQueue,
      { queueRowId }
    );
    if (result.matchId && result.templateId) {
      await bindCasualMatchSeedAfterOpen(ctx, result.matchId, result.templateId);
    }
    return { ok: true as const };
  },
});
...
export const expireCasualMatchQueueEntry = internalMutation({
  args: { queueRowId: v.id("casual_match_queue") },
  handler: async (ctx, { queueRowId }) => {
    const opened = await expireCasualMatchQueueRow(ctx, queueRowId);
    return {
      ok: true as const,
...
/** 排队超时开桌后同步 bind seed */
export const expireCasualMatchQueueEntryAndBind = internalAction({
  args: { queueRowId: v.id("casual_match_queue") },
  handler: async (ctx, { queueRowId }) => {
    const result = await ctx.runMutation(
      internal.service.tournament.join.casualMatchmaking.expireCasualMatchQueueEntry,
      { queueRowId }
    );
    if (result.matchId && result.templateId) {
      await bindCasualMatchSeedAfterOpen(ctx, result.matchId, result.templateId);
  ...
  handler: async (ctx, { uid, tournamentId, dailySoloCostAck }): Promise<JoinCasualRunResult> => {
    ...
    if (effectiveHumans === 1) {
      await ctx.scheduler.runAfter(
        CASUAL_SOLO_ASYNC_OPEN_DELAY_MS,
        internal.service.tournament.join.casualMatchmaking.openSoloTableFromScheduledQueueAndBind,
        { queueRowId }
      );
    } else {
      if (expiresAt != null) {
        await ctx.scheduler.runAfter(
          CASUAL_MATCH_QUEUE_TIMEOUT_MS,
          internal.service.tournament.join.casualMatchmaking.expireCasualMatchQueueEntryAndBind,
          { queueRowId }
        );
      }
      await ctx.scheduler.runAfter(
        0,
        internal.service.tournament.join.casualMatchmaking.processCasualMatchQueueForTemplateAndBind,
        { templateId: tournamentId }
      );
    }
