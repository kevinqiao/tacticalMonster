...
/** 与 `seedShopSkusIfEmpty` 一致：三档钻→币 + 三档法币→钻（展示） */
const SHOP_SKU_FALLBACK: ShopSkuSeed[] = [
  {
...
function mapSkuRow(r: ShopSkuSeed) {
  return {
    skuId: r.skuId,
    title: r.title,
...
export const listActiveShopSkus = query({
  ...
  handler: async (ctx) => {
    ...
    return SHOP_SKU_FALLBACK.map(mapSkuRow);
  },
});
...
export const seedShopSkusIfEmpty = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const s of SHOP_SKU_FALLBACK) {
