...
import { FIXED_CHEST_TABLES, SEASON_SHELF_SKUS } from "../data/casualSeasonShelfCatalog";
...
export const openFixedChest = mutation({
  ...
  handler: async (ctx, { uid, chestId }) => {
    if (SEASON_SHELF_SKUS.some((s) => s.chestId === chestId)) {
      return { ok: false as const, error: "chest_via_shelf_redeem_only" };
    }
    const table = FIXED_CHEST_TABLES[chestId];
