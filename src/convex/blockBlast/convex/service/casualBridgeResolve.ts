...
const CASUAL_BRIDGE_FETCH_TIMEOUT_MS = 90_000;
...
export async function fetchCasualMatchSubmitContext(args: {
  ...
  try {
    res = await fetch(url, {
      ...
      signal: AbortSignal.timeout(CASUAL_BRIDGE_FETCH_TIMEOUT_MS),
      body: JSON.stringify({
        uid: args.uid,
    ...
    console.error("[blockBlast] casual resolve fetch failed", e);
