import { Card } from "../../types/SoloTypes";
import type {
  BandThresholds,
...
export function layoutFingerprint(cards: Card[]): string {
  const parts = cards
    .map((c) => {
      const suit = c.suit ?? "?";
      const rank = c.rank ?? "?";
      return `${suit}:${rank}@${c.zoneId}:${c.zoneIndex}:${c.isRevealed ? 1 : 0}`;
    })
    .sort();
...
export function buildTierIndex(
  ...
  const tiers: Record<SolitaireSeedTier, SeedTierReportEntry[]> = {
    easy: [],
    medium: [],
    hard: [],
  };
