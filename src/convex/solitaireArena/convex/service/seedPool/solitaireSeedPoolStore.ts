...
export type SeedPoolEntryDoc = Doc<"solitaire_seed_pool_entries">;
export type RolloutSummaryDoc = Doc<"solitaire_seed_pool_rollout_summaries">;
...
export async function getActivePoolMeta(db: DbReader) {
  const rows = await db
    .query("solitaire_seed_pool_meta")
...
export async function listSeedEntries(
  ...
  let q = db.query("solitaire_seed_pool_entries");
  if (opts.tier != null) {
