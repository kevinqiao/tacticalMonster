...
/**
 * 赛季锦标赛处理器
 * 处理 seasonal_uno_championship 等赛季锦标赛
 */
export const seasonalHandler: TournamentHandler = {
    ...
    async join(ctx, { uid, gameType, tournamentType, player, season }) {
        ...
        // 查找或创建赛季锦标赛
        let tournament = await this.findOrCreateSeasonalTournament(ctx, {
            uid,
            gameType,
            tournamentType,
            player,
    ...
    async settle(ctx, tournamentId) {
        ...
        for (const playerData of sortedPlayers) {
            try {
                ...
                await ctx.db.insert("error_logs", {
                    ...
                    context: "seasonal_tournament_settle",
                    uid: playerData.uid,
                    createdAt: now.iso
                });
            }
        ...
        console.log(`赛季锦标赛 ${tournamentId} 结算完成`);
    }
};

// 辅助函数
async function findOrCreateSeasonalTournament(ctx: any, params: {
    uid: string;
    gameType: string;
    ...
    if (!tournament) {
        ...
        const tournamentId = await ctx.db.insert("tournaments", {
            ...
            createdAt: now.iso,
            updatedAt: now.iso,
            endTime: new Date(now.localDate.getTime() + (config.duration || 90 * 24 * 60 * 60 * 1000)).toISOString(),
            seasonalChallenge: {
                seasonId: season._id,
...
async function handleSingleMatchTournament(ctx: any, params: {
    ...
    const matchId = await MatchManager.createMatch(ctx, {
        ...
        gameType,
        matchType: "seasonal_single_match",
        maxPlayers: 1,
        minPlayers: 1,
        gameData: {
            player: {
