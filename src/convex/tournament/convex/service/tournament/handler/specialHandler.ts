...
/**
 * 特殊锦标赛处理器
 * 处理 special_holiday_event 等特殊锦标赛
 */
export const specialHandler: TournamentHandler = {
    ...
    async join(ctx, { uid, gameType, tournamentType, player, season }) {
        ...
        if (config.schedule?.startTime && config.schedule?.endTime) {
            ...
            const currentTime = now.localDate;

            if (currentTime < startTime || currentTime > endTime) {
                throw new Error("特殊活动不在有效时间内");
            }
        }
        ...
        // 查找或创建特殊锦标赛
        let tournament = await this.findOrCreateSpecialTournament(ctx, {
            uid,
            gameType,
            tournamentType,
            player,
    ...
    async settle(ctx, tournamentId) {
        ...
        for (const playerData of sortedPlayers) {
            try {
                ...
                await ctx.db.insert("error_logs", {
                    ...
                    context: "special_tournament_settle",
                    uid: playerData.uid,
                    createdAt: now.iso
                });
            }
        ...
        console.log(`特殊锦标赛 ${tournamentId} 结算完成`);
    }
};

// 辅助函数
async function findOrCreateSpecialTournament(ctx: any, params: {
    uid: string;
    gameType: string;
    ...
    if (!tournament) {
        ...
        const tournamentId = await ctx.db.insert("tournaments", {
            ...
            specialEvent: {
                eventType: "holiday",
                eventName: config.name || "特殊活动",
                startDate: config.schedule?.startTime,
                endDate: config.schedule?.endTime,
                type: "special"
            }
        });
...
async function handleSingleMatchTournament(ctx: any, params: {
    ...
    const matchId = await MatchManager.createMatch(ctx, {
        ...
        gameType,
        matchType: "special_single_match",
        maxPlayers: 1,
        minPlayers: 1,
        gameData: {
            player: {
