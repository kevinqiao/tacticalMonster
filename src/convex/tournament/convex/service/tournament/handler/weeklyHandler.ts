...
export const weeklyHandler: TournamentHandler = {
    ...
    async join(ctx, { uid, gameType, tournamentType, player, season }) {
        ...
        // 查找或创建每周锦标赛
        let tournament = await this.findOrCreateWeeklyTournament(ctx, {
            uid,
            gameType,
            tournamentType,
            player,
    ...
    async settle(ctx, tournamentId) {
        ...
        for (const playerData of sortedPlayers) {
            try {
                ...
                await ctx.db.insert("error_logs", {
                    ...
                    context: "weekly_tournament_settle",
                    uid: playerData.uid,
                    createdAt: now.iso
                });
            }
        ...
        console.log(`每周锦标赛 ${tournamentId} 结算完成`);
    }
};

// 辅助函数
async function findOrCreateWeeklyTournament(ctx: any, params: {
    uid: string;
    gameType: string;
    ...
    const { uid, gameType, tournamentType, player, season, config, now, weekStart } = params;
    ...
    let tournament = await ctx.db
        ...
        .filter((q: any) => {
            ...
            const tournamentWeekStart = getWeekStart(new Date(tournament).toISOString().split("T")[0]);
