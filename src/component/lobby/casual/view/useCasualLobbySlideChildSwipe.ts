...
/** 与 `useLobbySlideChildSwipe` 同逻辑，用于 casual 子页横滑切换 */
export function useCasualLobbySlideChildSwipe(
  ...
  useEffect(() => {
    ...
    const endGesture = (e: PointerEvent) => {
      ...
      const idx = CASUAL_LOBBY_CHILD_URIS.findIndex(
        (u) => normalizePageUri(u) === currentUri
      );
      if (idx < 0) return;

      const nextIdx = dx < 0 ? idx + 1 : idx - 1;
      if (nextIdx < 0 || nextIdx >= CASUAL_LOBBY_CHILD_URIS.length) return;

      openPage({ uri: CASUAL_LOBBY_CHILD_URIS[nextIdx] });
    };
