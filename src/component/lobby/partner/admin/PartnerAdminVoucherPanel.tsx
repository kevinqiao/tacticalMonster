  ...
  const save = async () => {
    ...
    try {
      await upsertPartnerShopSku({
        partnerId, kind: "voucher", skuId: id, title, priceCoins: Number(priceCoins),
        voucherRewardText: rewardText, listInShop,
      });
      setSkuId(""); setTitle(""); setPriceCoins("0"); setRewardText(""); setListInShop(true);
      setNote("兑换券 SKU 已保存。");
      await refresh();
    } catch (error) {
  ...
  return (
    <section>
      ...
      <div className="merchant-form-grid">
        <label>SKU ID（留空自动生成）<input value={skuId} onChange={(e) => setSkuId(e.target.value)} /></label>
        ...
        <label>兑换说明<input value={rewardText} onChange={(e) => setRewardText(e.target.value)} /></label>
        <label><input type="checkbox" checked={listInShop} onChange={(e) => setListInShop(e.target.checked)} /> 商店上架（关闭后仅可活动发放）</label>
      </div>
      <button type="button" className="merchant-btn" disabled={!title.trim()} onClick={() => void save()}>保存兑换券 SKU</button>
      {note ? <p className="merchant-note">{note}</p> : null}
      <ul className="merchant-note">
        {rows.map((row) => <li key={row.skuId}>
          <strong>{row.title}</strong> · {row.priceCoins} 金币 · {row.listInShop ? "上架" : "仅活动"} · {row.active ? "启用" : "停用"}
