...
const ServiceChargeCustom: React.FC<Props> = ({ type, onCancel, onComplete }) => {
  ...
  const done = useCallback(() => {
    setAmount(0);
    const service: ServiceCharge = { id: "####" };
    if (type === 2) service["amount"] = amount;
    else service["percent"] = amount;
    onComplete(service);
  }, [amount]);
...
export default ServiceChargeCustom;
