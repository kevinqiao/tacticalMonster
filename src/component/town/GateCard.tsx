...
const GateCard: React.FC<GateCardProps> = ({
  ...
  const canEnter = !loading && !insufficient && selection.ssaKey === "solitaire";

  const tierRows = useMemo(
    () =>
      buildingConfig.tiers.map((tier) => {
        const locked = !unlockedTierIds.includes(tier.id);
        return { ...tier, locked };
      }),
  ...
  const handleTierClick = useCallback(
    (tierId: string, locked: boolean, label: string, buyIn: number) => {
      if (locked) {
    ...
    <div className="town-gate-overlay" onClick={onClose}>
      <div className="town-gate-card" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        <div className="town-gate-card__header">
          <div className="town-gate-card__hero">{selection.buildingName.includes("Solitaire") ? "🎴" : "🃏"}</div>
          <div>
            ...
            <div className="town-chip-row">
              {tierRows.map((tier) => (
                <button
                  ...
                  onClick={() => handleTierClick(tier.id, tier.locked, tier.label, tier.buyIn)}
                >
