...
describe("stage mode & team preset (target state)", () => {
    ...
    it("tutorial chain: override + one_time_clear + hideTeamLayout + skills ⊆ allowedSkillIds", () => {
        for (const ruleId of TUTORIAL_RULE_IDS) {
            ...
            expect(getTournamentConfig(ruleId)?.matchRules?.modeType, ruleId).toBe("tutorial");
    ...
    it("solo challenge rules use score_tiers and teamPreset none", () => {
        ...
        expect(getTournamentConfig("monster_rumble_arena_bronze")?.matchRules?.modeType).toBe("solo_challenge");
