...
export const useWalkAction = (
    ...
    const walk = useCallback(async (to: { q: number; r: number }): Promise<void> => {
        ...
        const turnKey = `${character.uid}-${character.monsterId}-${(character as any).minionId ?? (character as any).bossId ?? ""}`;
            ...
            playWalk(character, path, async () => {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        const shouldEndTurn =
                            result.endTurn === true ||
                            (result.phaseChanges && result.endTurn !== false && pathSteps >= remainingSteps);
                        if (shouldEndTurn && result.phaseChanges) {
