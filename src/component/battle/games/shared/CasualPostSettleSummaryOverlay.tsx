...
export const CasualPostSettleSummaryOverlay: React.FC<CasualPostSettleSummaryOverlayProps> = ({
  ...
  const resolvedTitle =
    title ??
    (showTable ? t('postSettle.titleTable') : t('postSettle.titleSettled'));
  const resolvedDismiss = dismissLabel ?? t('postSettle.continue');
  const resolvedReplayLabel = replayLabel ?? getCasualAdReplayButtonLabel();

  const defaultSub = showTable
    ? triathlonSessionReplay
      ? t('postSettle.bodyTriathlonReplay')
      : t('postSettle.bodyWithTable')
    : showPending
      ? t('postSettle.bodyPendingPeers')
      : t('postSettle.bodySubmitted');
  ...
  let replayBtnText = resolvedReplayLabel;
  if (isAdReplay && !midgameReady) {
    replayBtnText = t('postSettle.adLoading');
  } else if (replayBusy) {
    replayBtnText = t('postSettle.matching');
  }
  if (countdown) {
    replayBtnText = `${replayBtnText} ${countdown}`;
  }

  const replayButtonTitle = [resolvedReplayLabel, quotaBadge].filter(Boolean).join(' ');
        ...
        <div className={pinFooter ? 'ssc ssc--pinnedFooter' : 'ssc'}>
          <div className={pinFooter ? 'ssc__scroll' : undefined}>
            ...
            {showPending ? (
              <p className="ssc__body msc-pendingPeersNote" role="status">
                {t('postSettle.pendingPeersHint')}
              </p>
            ) : null}
            {leagueLines.length > 0 ? (
              <ul
                className="ssc__leagueLines"
                aria-label={t('postSettle.weeklyLeagueAria')}
              >
          ...
          >
            ...
            <button
              ...
              onClick={onDismiss}
            >
              {midgameReady ? resolvedDismiss : t('postSettle.adLoading')}
            </button>
          </div>
        </div>
      </div>
