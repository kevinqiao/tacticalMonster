#!/usr/bin/env node
/**
 * 重算 index.json 内一个或全部 seed 的 rolloutSummaries（+ metrics）。
 * 可选 --sync 写回 Convex rollout 子表。
 *
 *   npx tsx scripts/blockblast/regen-rollout-summaries.mjs --seed blockblast-pool:v1:379
 *   npx tsx scripts/blockblast/regen-rollout-summaries.mjs --seed blockblast-pool:v1:379 --sync
 *   npx tsx scripts/blockblast/regen-rollout-summaries.mjs --all --in scripts/blockblast/output/pool-v1
 */
...
function parseArgs(argv) {
  const opts = {
    inDir: path.join(repoRoot, "scripts/blockblast/output/pool-v1"),
    seedId: "",
    all: false,
    sync: false,
    matchSeconds: 300,
...
async function regenOneSeed(seedId, rolloutCount, matchSeconds, thinkTimeScale, modules) {
  const { simulateSeedRollouts, verifyAllRollouts, toRolloutSummaries } = modules;
  const simOpts = {
    matchSeconds,
    keepDuplicateRollouts: true,
    ...(thinkTimeScale > 0 ? { thinkTimeScale } : {}),
  };
  const { allRollouts, metrics } = simulateSeedRollouts(seedId, rolloutCount, simOpts);
  const verify = verifyAllRollouts(
    seedId,
    allRollouts,
    matchSeconds,
    thinkTimeScale > 0 ? thinkTimeScale : undefined
  );
  if (!verify.ok) {
...
async function main() {
  ...
  const simPath = path.join(
    repoRoot,
    "src/convex/blockBlast/convex/service/seedPool/blockBlastSeedSimulator.ts"
  );
  const verifyPath = path.join(
    repoRoot,
    "src/convex/blockBlast/convex/service/seedPool/blockBlastSeedPoolReplayVerify.ts"
  );
  ...
  for (const seedId of targetIds) {
    ...
    const k = entry.metrics?.rolloutCount ?? raw.rolloutCount ?? 24;
    console.log(`regen summaries ${seedId} rollouts=${k} matchSeconds=${opts.matchSeconds}`);
    const result = await regenOneSeed(seedId, k, opts.matchSeconds, opts.thinkTimeScale, modules);
    if (!result.ok) {
  ...
  const rolloutCount = tiered[0]?.metrics.rolloutCount ?? raw.rolloutCount ?? 24;
  const nextIndex = {
    ...raw,
    poolVersion,
    rolloutCount,
