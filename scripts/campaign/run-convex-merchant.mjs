 ...
 */
export function runConvexMerchant(functionRef, args, convexArgs = []) {
  const cwd = MERCHANT_CONVEX_PROJECT_DIR;
  ...
  if (result.error) {
    throw new Error(
      `${result.error.message}\n(hint: ensure Node/npm on PATH; merchant cwd=${MERCHANT_CONVEX_PROJECT_DIR})`
    );
  }
  if (result.status !== 0) {
