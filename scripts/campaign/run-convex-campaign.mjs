 ...
 */
export function runConvexCampaign(functionRef, args, convexArgs = []) {
  const cwd = CAMPAIGN_CONVEX_PROJECT_DIR;
  ...
  if (result.error) {
    throw new Error(
      `${result.error.message}\n(hint: ensure Node/npm on PATH; campaign cwd=${CAMPAIGN_CONVEX_PROJECT_DIR})`
    );
  }
  if (result.status !== 0) {
